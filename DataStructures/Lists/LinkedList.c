...
struct Node * create_node_linkedlist(void *data, int size);
void destroy_node_linkedlist(struct Node *node_to_destroy);
struct Node * iterate_linkedlist(struct LinkedList *linked_list, int index);
...
void insert_node(struct LinkedList *linked_list, int index, void *data, int size){
    struct Node *node_to_insert = create_node_linkedlist(data, size);
    if(index == 0){
        node_to_insert->next = linked_list->head;
        linked_list->head = node_to_insert;
    } else {
        struct Node *cursor = iterate_linkedlist(linked_list, index - 1);
...
void remove_node(struct LinkedList *linked_list, int index){
    if(index == 0){
      ...
      destroy_node_linkedlist(node_to_remove);
    } else {
        struct Node *cursor = iterate_linkedlist(linked_list, index - 1);
...
void * retrieve_data(struct LinkedList *linked_list, int index){
    struct Node *cursor = iterate_linkedlist(linked_list, index);
    return cursor->data;
};
